//! Transactional mail boundary. Resend is the production implementation; the
//! file outbox is intentionally a local-development fallback.
...
impl EmailSender for DevelopmentOutbox {
    async fn send(&self, message: EmailMessage) -> Result<(), EmailError> {
        ...
        let file = self.root.join(format!("{}-{}.json", Utc::now().timestamp_millis(), Uuid::new_v4()));
