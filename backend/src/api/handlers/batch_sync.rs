...
use crate::{api::{error::ApiError, handlers::sessions::database_session_by_code}, state::AppState};

#[derive(Debug, Deserialize)]
pub struct BatchSyncRequest {
    pub session_code: String,
    #[serde(default)] pub captions: Vec<OfflineCaption>,
    #[serde(default)] pub presence_heartbeats: Vec<OfflineHeartbeat>,
}

#[derive(Debug, Deserialize)]
pub struct OfflineCaption { pub text: String, pub timestamp: DateTime<Utc> }

#[derive(Debug, Deserialize)]
pub struct OfflineHeartbeat { pub matric: String, pub timestamp: DateTime<Utc> }

#[derive(Debug, Serialize)]
pub struct BatchSyncResponse { pub processed_captions: i64, pub processed_heartbeats: i64, pub sync_status: &'static str }

pub async fn sync(
    State(state): State<AppState>, Json(input): Json<BatchSyncRequest>,
    ...
    for caption in input.captions {
        ...
        if text.is_empty() || text.len() > 4000 { continue; }
    ...
    Ok((StatusCode::OK, Json(BatchSyncResponse { processed_captions, processed_heartbeats, sync_status: "complete" })))
}
