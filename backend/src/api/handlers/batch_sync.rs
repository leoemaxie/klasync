...
pub async fn sync(
    ...
    for caption in input.captions {
        ...
        let caption_id = uuid::Uuid::now_v7();
        sqlx::query!(
            "insert into caption_chunks (id, session_id, sequence_number, text, created_at) values ($1, $2, (select coalesce(max(sequence_number), 0) + 1 from caption_chunks where session_id = $2), $3, $4)",
            caption_id,
            session.id,
            text,
            caption.timestamp
    ...
    for heartbeat in input.presence_heartbeats {
        let matric = heartbeat.matric.trim();
        let updated = sqlx::query!(
            "update session_participants set last_seen_at = greatest(last_seen_at, $3), heartbeat_count = heartbeat_count + 1 where session_id = $1 and lower(matric_number) = lower($2) and removed_at is null",
            session.id,
            matric,
            heartbeat.timestamp
        )
