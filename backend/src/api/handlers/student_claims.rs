...
pub async fn request(
    ...
    state
        ...
        .await
        .log_internal_error("Failed to send claim verification email")?;
    Ok((
        StatusCode::ACCEPTED,
...
pub async fn verify(
    ...
    if record.consumed_at.is_some() || record.expires_at <= Utc::now() || record.attempts >= 5 {
        return Err(ApiError::unauthorized(
            "Verification code has expired or maximum attempts reached",
        ));
    }
...
fn validate_university_email(email: &str) -> Result<(), ApiError> {
    let domain = std::env::var("UNIVERSITY_EMAIL_DOMAIN")
        .ok()
