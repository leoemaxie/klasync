...
pub async fn resolve(
    ...
    let session = sqlx::query_as!(
        LectureSession,
        r#"select session.id as "id!", session.course_id as "course_id!", session.title as "title!", session.short_code as "short_code!", session.invite_token as "invite_token!", session.status as "status!: SessionStatus", coalesce(session.started_at, now()) as "started_at!"
