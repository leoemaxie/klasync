...
pub async fn upload(
    ...
    if !["audio", "recording", "transcript", "notes"].contains(&resource_type.as_str()) {
        return Err(ApiError::bad_request("Invalid resource type for file upload"));
    }
    let file = multipart
        .next_field()
        .await
    ...
    let bytes = file.bytes().await.map_err(|_| ApiError::bad_request("Failed to read uploaded file"))?;
