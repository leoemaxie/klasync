...
pub async fn heartbeat(
    ...
    if let Some(redis) = &state.redis {
        if let Err(error) = redis.set_presence(&participant.1.to_string(), &participant.0.to_string(), TTL).await {
    ...
    Ok((StatusCode::OK, Json(PresenceHeartbeat { participant_id: participant.0, heartbeat_count: participant.2, presence_ttl_seconds: TTL })))
}
