...
pub async fn rate_limit(State(state): State<AppState>, request: Request, next: Next) -> Response {
    ...
    if let Some(redis) = &state.redis {
            ...
            .await
        {
            ...
            Err(error) => {
                tracing::warn!(%error, "Managed Redis rate limiter unavailable; using local fallback")
            }
        }
    }
...
fn policy(path: &str) -> Option<(&'static str, u32, Duration)> {
    if path.contains("/auth/password-reset") {
        return Some(("password-reset", 5, Duration::from_secs(900)));
    }
    if path.contains("/auth/") {
        return Some(("auth", 12, Duration::from_secs(60)));
    }
    if path.contains("/students/claims") {
        return Some(("claims", 5, Duration::from_secs(300)));
    }
    if path.contains("/join") {
        return Some(("join", 30, Duration::from_secs(60)));
    }
    if path.contains("/ai-jobs") {
        return Some(("ai", 10, Duration::from_secs(60)));
    }
    if path.contains("/upload") {
        return Some(("upload", 20, Duration::from_secs(60)));
    }
    None
}
