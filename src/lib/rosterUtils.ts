...
export function parseRosterTextToStudents(rawText: string): RosterStudent[] {
  const cleanedText = rawText.replace(/^\uFEFF/, "").replace(/[\u200B-\u200D\uFEFF]/g, "");
  ...
  const hasHeaderMatric = headerCols.some((c) => c.includes("matric") || c.includes("id") || c.includes("reg"));
  const hasHeaderName = headerCols.some((c) => c.includes("name") || c.includes("student"));

  if (hasHeaderMatric || hasHeaderName) {
    startIdx = 1; // Skip header
    headerCols.forEach((col, idx) => {
      if (col.includes("matric") || col.includes("id") || col.includes("reg")) matricCol = idx;
...
export function importFile(state: SessionState, eventOrFile: Event | File) {
  ...
  if (eventOrFile instanceof File) {
    file = eventOrFile;
  } else if (eventOrFile && (eventOrFile.target as HTMLInputElement)?.files?.[0]) {
  ...
  if (!isCsvOrText && !isXlsx) {
    state.rosterNotice = "Unsupported file format. Please upload a .csv, .tsv, or .xlsx file.";
    return;
  }
  ...
  reader.onload = () => {
    try {
      ...
      state.rosterNotice = "Error reading file contents. Ensure the file contains readable student records.";
    }
  };
