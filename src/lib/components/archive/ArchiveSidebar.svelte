...
<aside class="studio-sidebar panel">
  <div class="sidebar-header">
    ...
    <div class="search-input-wrap">
      ...
      />
      {#if searchQuery}
        <button type="button" class="clear-query-btn text" onclick={() => (searchQuery = '')}>Clear</button>
      {/if}
    </div>
  </div>
      ...
      {#if filteredClaims.length > 0}
        <div class="claims-stack" role="list">
          {#each filteredClaims as claim (claim.id)}
            <ArchiveClaimItem {claim} isSelected={selectedClaim?.id === claim.id} onSelect={onSelectClaim} />
          {/each}
        </div>
      {:else}
        <div class="empty-sidebar-box"><p class="hint">No lectures match query</p></div>
      {/if}
    {:else}
  ...
  <div class="sidebar-footer">
    <div class="sync-status-row"><span class="sync-dot"></span><span class="sync-label">Sync Active</span></div>
  </div>
</aside>

<style>
  .studio-sidebar { display: flex; flex-direction: column; background: rgba(16, 9, 4, 0.5); border: 1px solid var(--color-cork-border); border-radius: var(--radius-cards); padding: var(--spacing-14); gap: var(--spacing-12); max-height: calc(100vh - var(--nav-height) - 160px); position: sticky; top: calc(var(--nav-height) + 20px); }
  .sidebar-header { display: flex; flex-direction: column; gap: var(--spacing-10); }
  ...
  .mode-btn { flex: 1; display: inline-flex; align-items: center; justify-content: center; gap: 6px; padding: 6px; font-size: 11px; text-transform: uppercase; background: transparent; border: none; color: var(--color-driftwood); border-radius: 3px; }
  ...
  .empty-sidebar-box, .course-directory-shortcut { padding: var(--spacing-14); text-align: center; }
  .sidebar-footer { border-top: 1px solid var(--color-cork-border); padding-top: var(--spacing-8); margin-top: auto; }
  ...
  @media (max-width: 960px) { .studio-sidebar { position: static; max-height: none; } }
</style>
