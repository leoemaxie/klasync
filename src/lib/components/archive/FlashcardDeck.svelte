<script lang="ts">
  ...
  let { sessionId = 'demo-session', transcript = '', cards = [] }: {
    sessionId?: string;
    transcript?: string;
    cards?: FlashcardItem[];
  } = $props();
...
<div class="panel flashcard-deck">
  <div class="deck-header">
    ...
    <div class="header-tools">
      ...
      <button type="button" class="tool-btn" title="Reset" onclick={resetDeck}><RotateCcw size={12} /></button>
      {#if deck.length}<span class="card-counter">{currentIndex + 1} / {deck.length}</span>{/if}
    </div>
  </div>
  ...
  {#if deck.length > 0}
    <div class="deck-progress"><div class="progress-fill" style="width: {((currentIndex + 1) / deck.length) * 100}%"></div></div>
...
<style>
  .flashcard-deck { display: flex; flex-direction: column; gap: var(--spacing-8); padding: var(--spacing-14); background: rgba(16, 9, 4, 0.4); border: 1px solid var(--color-cork-border); border-radius: var(--radius-cards); }
  .deck-header { display: flex; justify-content: space-between; align-items: center; }
  .header-tools { display: flex; align-items: center; gap: 8px; }
  .tool-btn { background: transparent; border: 1px solid var(--color-cork-border); color: var(--color-driftwood); padding: 4px; border-radius: 4px; cursor: pointer; display: inline-flex; align-items: center; }
  .tool-btn:hover { border-color: var(--color-warm-cream); color: var(--color-warm-cream); }
  .card-counter { font-size: 10px; letter-spacing: 0.1em; color: var(--color-driftwood); font-family: var(--font-mono, monospace); }
  .deck-progress { height: 2px; background: rgba(255, 237, 215, 0.1); border-radius: 2px; overflow: hidden; }
  .progress-fill { height: 100%; background: var(--color-warm-cream); transition: width 0.3s ease; }
  .deck-actions { display: flex; justify-content: space-between; gap: var(--spacing-8); margin-top: var(--spacing-2); }
