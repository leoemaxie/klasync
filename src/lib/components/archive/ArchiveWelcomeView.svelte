...
<div class="welcome-studio-state">
  ...
  <p class="welcome-desc">
    ...
    synchronized transcripts, AI topic breakdowns, audio replays, and flashcards.
  </p>
  ...
  {#if claims.length > 0}
    <div class="start-action">
      <button type="button" class="primary" onclick={() => onOpenLatest(claims[0])}>
...
<style>
  .welcome-studio-state { display: flex; flex-direction: column; align-items: center; text-align: center; padding: var(--spacing-28) var(--spacing-14); gap: var(--spacing-12); max-width: 680px; margin: 0 auto; }
  ...
  .welcome-title { font-family: var(--font-display); font-size: 26px; color: var(--color-warm-cream); margin: 0; }
  .welcome-desc { font-size: 13px; color: var(--color-driftwood); line-height: 1.6; margin: 0; }
  .quick-overview-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--spacing-10); width: 100%; margin-top: var(--spacing-8); text-align: left; }
  ...
  .start-action { margin-top: var(--spacing-6); }
