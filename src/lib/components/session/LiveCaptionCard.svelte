...
<article class="panel caption-card" class:dyslexic-mode={dyslexicFont} style="font-size: {fontSize}; line-height: {lineHeight};" aria-live="polite">
  ...
  <div class="caption-content">
    {#if captions.length === 0}
      <div class="caption-empty-box">
        ...
        <span class="empty-hint">Captions will appear automatically as the lecturer speaks.</span>
      </div>
    {:else}
...
<style>
  .caption-card { display: flex; flex-direction: column; gap: var(--spacing-12); min-height: 220px; justify-content: space-between; height: 100%; }
  ...
  .caption-counter { font-size: 10px; color: var(--color-warm-cream-dim); text-transform: uppercase; font-family: var(--font-mono, monospace); }
  .caption-content { flex: 1; display: flex; align-items: center; padding: 8px 0; }
  .caption-empty-box { display: flex; flex-direction: column; gap: 4px; }
  .empty-prompt { font-size: 15px; font-weight: 700; color: var(--color-warm-cream); }
  ...
  .dyslexic-mode .caption-text { font-family: 'OpenDyslexic', 'Comic Sans MS', sans-serif; letter-spacing: 0.05em; }
  .caption-next-btn { align-self: flex-start; font-size: 11px; }
