...
<div class="session-top-card">
  ...
  <div class="live-status-pills">
    {#if isPaused}
      <span class="status-pill paused"><Pause size={12} aria-hidden="true" /> ROOM PAUSED</span>
    {:else}
      <span class="status-pill broadcasting"><span class="live-dot" aria-hidden="true"></span> LIVE</span>
    {/if}
    ...
    {#if isRecording}
      <span class="status-pill recording active-rec">
        ...
        {#if audioLevel > 0}
          <span class="level-indicator-bar" style="width: {Math.max(10, Math.min(60, audioLevel))}px"></span>
        {/if}
      </span>
    {:else}
      <span class="status-pill idle"><Mic size={12} aria-hidden="true" /> MIC STANDBY</span>
    {/if}
  </div>
</div>
...
<style>
  ...
  .status-pill { font-size: 11px; font-weight: 600; letter-spacing: 0.08em; padding: 6px 12px; border-radius: 20px; display: inline-flex; align-items: center; gap: 6px; text-transform: uppercase; }
  .status-pill.broadcasting { background: rgba(74, 183, 114, 0.12); color: var(--color-warm-cream); border: 1px solid rgba(74, 183, 114, 0.3); }
  .live-dot { width: 6px; height: 6px; border-radius: 50%; background: #4ab772; box-shadow: 0 0 8px #4ab772; animation: blink 1.5s infinite; }
  .status-pill.paused { background: rgba(220, 80, 0, 0.12); color: var(--color-ember-accent); border: 1px solid rgba(220, 80, 0, 0.3); }
  .status-pill.recording.active-rec { background: rgba(239, 68, 68, 0.2); color: var(--color-warm-cream); border: 1px solid #ef4444; box-shadow: 0 0 16px rgba(239, 68, 68, 0.35); }
  ...
  .level-indicator-bar { height: 4px; background: #ef4444; border-radius: 2px; transition: width 0.1s ease; box-shadow: 0 0 6px #ef4444; }
  .status-pill.idle { background: rgba(56, 36, 22, 0.6); color: var(--color-driftwood); border: 1px solid var(--color-cork-border); }
