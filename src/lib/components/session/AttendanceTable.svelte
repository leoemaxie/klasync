      ...
      {#each filteredParticipants as participant (participant.id || participant.matric)}
        <tr class="table-row">
          ...
          <td>
            {#if participant.verified}
              <span class="badge verified"
                ><ShieldCheck size={12} aria-hidden="true" /> VERIFIED MATCH</span
              >
            {:else}
              <span class="badge provisional"
                ><UserX size={12} aria-hidden="true" /> PROVISIONAL GUEST</span
              >
            {/if}
          </td>
          <td>
          ...
          <td class="time-cell"
            ...
            {formatJoinedTime(participant.joinedAt)}</td
          >
        </tr>
      {/each}
    </tbody>
...
<div class="footer-count">
  Showing {filteredParticipants.length} of {totalCount} participant record{totalCount ===
  1
    ? ''
    : 's'}
</div>
