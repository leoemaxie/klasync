<script lang="ts">
  ...
  async function exportGradebookReport() {
    ...
    try {
      ...
      const activeCourse =
        courseCode.trim() || summary?.course_code || 'COURSE';
      const rows: string[] = [
        'Course Code,Matric Number,Student Name,Verification Status,Heartbeat Count',
      ];
      const studentsToExport =
        participants.length > 0
          ? participants.map(
              (p) =>
                `${activeCourse},"${p.matric}","${p.name}",${p.verified ? 'Verified Match' : 'Provisional'},${p.heartbeats}`
            )
          : roster.map(
              (r) =>
                `${activeCourse},"${r.matric}","${r.name}",Registered Roster,0`
            );
      rows.push(
        ...(studentsToExport.length === 0
          ? [
              `${activeCourse},"N/A","No registered students","Pending Session Entry",0`,
            ]
          : studentsToExport)
      );
      ...
      link.setAttribute(
        'download',
        `official_gradebook_${activeCourse.replaceAll(' ', '_')}.csv`
      );
