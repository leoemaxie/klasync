<script lang="ts">
  let { value = "KLASYNC-ROOM", size = 128 }: { value: string; size?: number } = $props();
  ...
  const grid = $derived.by(() => {
    ...
    for (let r = 0; r < matrixSize; r++) {
      for (let c = 0; c < matrixSize; c++) {
        if (!cells[r][c] && (r > 7 || c > 7) && (r < 14 || c > 7) && (r > 7 || c < 14)) {
...
<div class="qr-svg-container" style="--size: {size}px;">
  <svg viewBox="0 0 21 21" xmlns="http://www.w3.org/2000/svg" class="qr-svg" aria-label="QR Code for {value}">
